package fr.maxlego08.zdrawer.zcore.utils.loader;

import fr.maxlego08.zdrawer.exceptions.ItemEnchantException;
import fr.maxlego08.zdrawer.exceptions.ItemFlagException;
import fr.maxlego08.zdrawer.zcore.logger.Logger;
import fr.maxlego08.zdrawer.zcore.logger.Logger.LogType;
...
public class ItemStackLoader extends ZUtils implements Loader<ItemStack> {
    ...
    public void save(ItemStack item, YamlConfiguration configuration, String path) {
        ...
        if (meta.getItemFlags().size() != 0)
            configuration.set(path + "flags", meta.getItemFlags().stream().map(flag -> flag.name()).collect(Collectors.toList()));
        if (meta.hasEnchants()) {
        ...
        if (NmsVersion.getCurrentVersion().isBarrel() && meta.hasCustomModelData()) {
